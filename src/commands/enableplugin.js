...
module.exports = class Ping extends Command {
    constructor(k) {
        super(k);
        this.name = 'enableplugin',
        this.description = 'enables a plugin for the server';
        this.admin = true;
        this.options = [
            {
                type: 3,
    ...
    execute(int) {
        ...
        if (!plugins.includes(plugin)) return this.reply(int, 'This plugin is already enabled');
        plugins.splice(plugins.indexOf(plugin), 1);
        this.db.guilds.setDisabledPlugins.run(plugins.join(',') || null, int.guild.id);
        return this.reply(int, `Successfully enabled ${plugin} plugin`);
    }
};